...
class LegacyBackendOptimizationManager {
   ...
   * Integrates with apiDeduplicator to prevent duplicate concurrent requests
   */
  private async executeWithDeduplication<T>(
    ...
    const dedupKey = cacheKey || `query_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    // Use apiDeduplicator to handle concurrent request deduplication
    return await apiDeduplicator.deduplicate(dedupKey, queryFn);
  }
