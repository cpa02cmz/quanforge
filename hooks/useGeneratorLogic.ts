...
import { Message, MessageRole, Robot, StrategyParams, StrategyAnalysis, BacktestSettings, SimulationResult, RobotUpdate } from '../types';
...
// Optimized parallel loading with Promise.all
  ...
  const handleSave = async () => {
      ...
      try {
        if (id) {
            await mockDb.updateRobot(id, robotData as RobotUpdate);
        } else {
            ...
            const { data } = await mockDb.saveRobot(newRobot);
