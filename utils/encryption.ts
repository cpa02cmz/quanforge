...
// Get encryption key from environment with secure fallback
const getEncryptionKey = (): string => {
  // Try production environment variable first
  if (typeof process !== 'undefined' && process.env?.['ENCRYPTION_KEY']) {
    return process.env['ENCRYPTION_KEY'];
  }
  
  // Try Vite environment variable for frontend
  ...
  console.warn('Using fallback encryption key - set ENCRYPTION_KEY or VITE_ENCRYPTION_KEY in production');
  return 'QuantForge_AI_Secure_Key_2024_Dev_Fallback';
};
