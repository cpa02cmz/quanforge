...
export const CodeEditor: React.FC<CodeEditorProps> = React.memo(({ code, readOnly = false, filename = "ExpertAdvisor", onChange, onRefine, onExplain }) => {
  ...
  useLayoutEffect(() => {
    if (!isEditing && codeRef.current && (window as unknown as { Prism?: { highlightElement: (element: HTMLElement) => void } }).Prism) {
        ...
        requestAnimationFrame(() => {
          (window as unknown as { Prism?: { highlightElement: (element: HTMLElement) => void } }).Prism!.highlightElement(codeRef.current!);
        });
    }
  }, [code, isEditing]);
