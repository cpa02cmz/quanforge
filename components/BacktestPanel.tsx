            ...
            <div className="bg-dark-surface border-b border-dark-border p-4">
                <div className="flex flex-wrap gap-4 items-end">
                    ...
                    <button
                        ...
                        aria-live="polite"
                        aria-busy={isRunning}
                        aria-label={isRunning ? 'Running backtest simulation' : 'Run backtest simulation'}
                    >
                        {isRunning ? (
                             <svg className="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" aria-hidden="true"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                        ) : (
                        ...
                        <span aria-hidden="true">{t('bt_run')}</span>
                    </button>
                    ...
                    {result && (
                        <button
                            ...
                            aria-label={`Export simulation results to CSV - Final balance: $${result.finalBalance.toLocaleString()}, Total return: ${result.totalReturn >= 0 ? '+' : ''}${result.totalReturn}%`}
                        >
